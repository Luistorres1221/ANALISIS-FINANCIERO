...
export class AppStatusService {
  ...
  private deriveSymbolsFromDataset(dataset: DatasetRow[]): string[] {
    if (!dataset.length) {
      return [];
    }

    const symbols = Object.keys(dataset[0])
      .filter((key) => key !== 'date')
      .map((key) => {
        const match = key.match(/^(.*)_(open|close|high|low|volume)$/);
        return match?.[1];
      })
  ...
  loadSymbolsOnly(): void {
    this.api.getSymbols()
      ...
      .subscribe((s) => {
        this.symbols.set(s);
        if (s.length === 0 && this.dataset().length > 0) {
          this.symbols.set(this.deriveSymbolsFromDataset(this.dataset()));
        }
      });
  }
  ...
  private loadHeavyData() {
    ...
    this.syncState.set('refreshing');
    return forkJoin({
      symbols: this.api.getSymbols().pipe(catchError(() => of([]))),
      ...
      tap(({ symbols, dataset, analysis, correlation }) => {
        const resolvedSymbols = symbols.length ? symbols : this.deriveSymbolsFromDataset(dataset);
        this.symbols.set(resolvedSymbols);
